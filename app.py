...
def format_field(rows: int, cols: int, obstacles: Set[Tuple[int, int]], 
    ...
    # Prepare the representation for the teams (including the ball if in possession)
    a_str = team_a_emoji + (BALL_EMOJI if ball_owner == "A" else "")
    b_str = team_b_emoji + (BALL_EMOJI if ball_owner == "B" else "")
    
    # Place the teams on the field (even if they overlap the goal)
    r, c = team_a
    field[r][c] = a_str
    r, c = team_b
    field[r][c] = b_str
...
def get_team_move(team: str, team_pos: Tuple[int, int], opponent_pos: Tuple[int, int], obstacles: Set[Tuple[int,int]], 
    ...
    """
    Requests a move from the selected LLM for the given team,
    providing context about the current field state.
    """
    model_name = team_llm
    
    # Determine positions of both teams to display the full field
...
"""
    try:
        ...
        st.write(f"Error communicating with model {model_name}:", e)
...
def run_game(team_a_emoji: str, team_b_emoji: str, team_a_llm: str, team_b_llm: str):
    ...
    A goal is scored when the team in possession moves the ball into the cells of the designated goal area:
    ...
    field_placeholder.markdown(format_field(ROWS, COLS, obstacles, team_a, team_b, ball, ball_owner, team_a_emoji, team
_b_emoji))
    ...
    while turn < max_turns:
        ...
        # Request moves from each team using their selected LLM.
        move_a = get_team_move("A", team_a, team_b, obstacles, ROWS, COLS, ball, ball_owner, team_a_llm, team_a_emoji, 
team_b_emoji)
        move_b = get_team_move("B", team_b, team_a, obstacles, ROWS, COLS, ball, ball_owner, team_b_llm, team_a_emoji, 
team_b_emoji)
        ...
        if ball_owner is None:
            ...
            if a_adj and not b_adj:
                ...
                st.write(f"Both teams contest the ball. It is assigned to {team_a_emoji if ball_owner=='A' else team_b_
emoji}.")
        else:
        ...
        # Check for a goal using the defined goal area cells
        if ball_owner == "A" and ball in right_goal:
            st.markdown(f"### GOAL! {team_a_emoji} Team A ({team_a_llm}) scores.")
            st.balloons()
            field_placeholder.markdown(format_field(ROWS, COLS, obstacles, team_a, team_b, ball, ball_owner, team_a_emo
ji, team_b_emoji))
            break
...
ji, team_b_emoji))
        ...
        field_placeholder.markdown(format_field(ROWS, COLS, obstacles, team_a, team_b, ball, ball_owner, team_a_emoji, 
team_b_emoji))
        time.sleep(1)
    else:
